...
PIECE_NAMES   = ["Ettur", "Ratsu", "Oda", "Vanker", "Lipp", "Kuningas"]
...
def plot_pst_group(pst_dict, group_title, cmap, filename):
    ...
    for ax, key, name in zip(axes.flat, PIECE_KEYS, PIECE_NAMES):
        grid = pst_to_grid(pst_dict[key])
        vmax = np.abs(grid).max()

        im = ax.imshow(grid, cmap=cmap, vmin=-vmax, vmax=vmax, aspect="equal")
        ...
        ax.set_xticklabels(FILE_LABELS, color="#aaaacc", fontsize=9)
        ax.set_yticklabels(RANK_LABELS, color="#aaaacc", fontsize=9)
        ax.tick_params(length=0)

        for spine in ax.spines.values():
            spine.set_edgecolor("#444466")

        ax.set_title(f"{name}", color="white", fontsize=13,
                     fontweight="bold", pad=8, fontfamily="DejaVu Serif")
...
plot_pst_group(
    PST_MG,
    "Keskmängu nupp-ruutlauad",
    cmap="RdYlGn",
    filename="C:/Users/Kaare/Desktop/pst_middlegame.png",
)
